...
#asm("sei")     }
    while (1){ 
        if(temp[1]>temp[2]){mnha=temp[1]-temp[2];
           }else{mnha=temp[2]-temp[1];}
        mean=(temp[1]+temp[2])/2;
        lcd_gotoxy(0,0);
        sprintf(line,"T1=%-2dT2=%-2dtl=%-2dme=%-2d",temp[1],temp[2],mnha,mean);
        lcd_puts(line); 
        lcd_gotoxy(0,1);
        sprintf(line,"T=%-5d",T);
        lcd_puts(line);
        if(mean<38){PORTD.1=1;}
        if(mean>40){PORTD.1=0;} 
        if(mnha>3){PORTD.0=1;}
        if(mnha<2){PORTD.0=0;}    
        delay_ms(10);
        T++;
        ...
        if(STMO==1&&!PINA.3){PORTD.2=0;STMO=0;}
        if(STMO==2&&!PINA.2){PORTD.3=0;STMO=0;}          
    }
}

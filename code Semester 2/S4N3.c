...
#asm("sei")                            }
    ...
    while(1){
        if(!PINA.2){ste=1;PORTD.2=1;lcd_gotoxy(3,1);lcd_putsf("System ON ");}
        if(!PINA.3){ste=0;PORTD.2=0;lcd_gotoxy(3,1);lcd_putsf("System OFF ");}
        t1=adc_data[0]/2;
        t2=16+adc_data[1]/64; 
        akh =t1-t2;
        if(ste){
        ...
        lcd_gotoxy(0,0);
        sprintf(line,"Temp=%-3dSET=%-3d", t1,t2);
        lcd_puts(line);

...
lcd_init(20);         }
    ...
    while(1){ 
        ...
        for(i=9;i>0;) {
            ...
            i--;}                    
        if(g==1){if(xx[0]==19)xx[0]=0; else xx[0]++;}
        if(g==2){if(xx[0]==0)xx[0]=19; else xx[0]--;};
        if(g==3){if(yy[0]==0)yy[0]=3;  else yy[0]--;};
        if(g==4){if(yy[0]==3)yy[0]=0;  else yy[0]++;}; 

        if(gaez_x==xx[0] && gaez_y==yy[0]){
            amteaz++;
            gaez_x=gaez_x+5;if(gaez_x>16)gaez_x=gaez_x-9;
            gaez_y++;if(gaez_y>3)gaez_y=gaez_y-2; 
            while(amteaz==6){
                lcd_gotoxy(6,1);
                lcd_putsf("you win");}    
        }else{
            lcd_gotoxy(gaez_x,gaez_y);
            lcd_putsf("#");}
